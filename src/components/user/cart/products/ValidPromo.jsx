...
class ValidPromo extends React.Component {
    ...
    render() {
        ...
        cartProducts = cartProducts.map((p, i) => {
            return <CartProductRow key={p.id + i}
                                   editable={false}
                                   data={p}/>;
        });
        ...
        if (this.props.selectedPresents.length > 0) {
            selectedPresents = this.props.selectedPresents.map((p, i) => {
                return <CartProductRowPresent key={p.id + i}
        ...
        return (
            <div id="cart-products-table">
                ...
                &&
                <h4>Можете да изберете <span
                    className="presents-number">{discountedProductsCount - selectedPresentsCount}</span>
                ...
                <h4>Успешно добавихте намалените продукти, които предоставя промоцията.
                    <small>За да промените избора си, моля, изтрийте от таблицата някой от добавените подаръци.</small>
                </h4>
                }
